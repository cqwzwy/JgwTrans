$(document).ready(function() {
		...
		var dataJson2;
		$.getJSON("json/汉字检索.json",function(data){
			dataJson2=data.result;
		})
		
		
			//点击事件
		$(".hz .hzs").bind("keypress",function(e){
			var value=$(".hzs").val()
			if(e.keyCode==13){
				getHz(value)
			}
		...
		$(".hz .submits").bind("click",function(){
			
			var value=$(".hzs").val()
			getHz(value)
		})
	
		function getHz(value){
			var resultSelect={result:[]}
			$.each(dataJson2,function(index,e){
					
				if(value==e.key1||value==e.key2)
					resultSelect.result.push(e);
			...
			var i=0;
			var resultShow="<span>共有结果数 ：<b>"+resultSelect.result.length+"</b></span></br>"
			$.each(resultSelect.result,function(index,e){
				var FontStr;
							
				resultShow+="<span class='InputValue' data-code='"+e.jgw
				+"' data-res='"+e.jgw+"' title='简体字："+e.key1+"\n繁体字："+e.key2+"' >"
				+e.jgw+"</span>";
